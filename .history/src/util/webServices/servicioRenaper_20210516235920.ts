import { renaper } from "../config.private";

const soap = require("soap");
const url = renaper.url;
const serv = renaper.serv;
const login = renaper.auth;

export function getServicioRenaper(persona, userInfo?) {
  console.log("ING", persona);
  let resultado: any;
        ...
        if (client) {
          if (persona.documento && persona.sexo) {
            client.LoginPecas(login, async (err2, result) => {
              ...
              const filtro =
                "documento=" + persona.documento + ";sexo=" + persona.sexo;
              try {
                resultado = await consultaRenaper(
                  result,
                  tipoConsulta,
                  filtro,
                  userInfo
        ...
        client2.Solicitar_Servicio(args2, (err4, result2) => {
          ...
          if (result2 && result2.return) {
            ...
            if (result2.return.Resultado["$value"]) {
              const resultado = new Buffer(
                result2.return.Resultado["$value"],
                "base64"
              );
